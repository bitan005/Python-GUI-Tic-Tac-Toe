...
clk = True      #clk set to True so that first X's turn

def btnclick(btn):      #Function on Button click
    global clk, move
...
def autoplay():
    global clk, move
    lst_options = [btn_1, btn_2, btn_3, btn_4, btn_5, btn_6, btn_7, btn_8, btn_9]
    comp_option = random.choice(lst_options)
    try:    
        if comp_option['text']=="":
                comp_option['text'] = "O"
                move+=1
        elif comp_option['text']=="X" or comp_option['text']=="O":
                autoplay()
